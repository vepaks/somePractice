function checkBudget(input) {
    let yearsOfLily = Number(input[0]),
        wadhingMashinePrice = Number(input[1]),
        toyPrice = Number(input[2]);
    ...
    for (let i = 1; i <= yearsOfLily; i++) {
        ...
        if (i % 2 === 0) {
            moneyaddet += 10;
            savedMoney = savedMoney + moneyaddet;
            stolenMoney += 1;
        } else {
            toysCounter ++;
        }
    ...
    savedMoney = (savedMoney + (toysCounter * toyPrice)) - stolenMoney;
    
    if (savedMoney >= wadhingMashinePrice) { 
       console.log(`Yes! ${(savedMoney - wadhingMashinePrice).toFixed(2)}`);
    } else {
        console.log(`No! ${(wadhingMashinePrice - savedMoney).toFixed(2)}`);
    }
...
// Числата N и M трябва да за форматирани до вторият знак след десетичната запет